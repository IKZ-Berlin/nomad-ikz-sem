...
def add_section(  # noqa: PLR0912, PLR0913, PLR0915
    yt,
    name,
    ...
    indentation_level=2,
    sub_section=False,
    default_unit_list=None,
    type_lut=type_lut,
    non_base_section_object=False,
    editables=None,
):
    ...
    elif not non_base_section_object:
...
def read_schema(filepath):
    "only for schemas with a depth of: sections, section-quantities, subsections,subsection-quantities"  # noqa: E501
            ...
            if i[4] != w1:
                ...
                if i[8:end] != 'eln' and i[8:end] != 'browser':
                    if 'sub_section' in lines[c - 1]:
                        ...
                        sections[selected_section]['sub_sections'][
                            selected_subsection
                        ] = dict()
                        sections[selected_section]['sub_sections'][selected_subsection][
                            'quantities'
                        ] = []
...
def get_metadata_tif(filepath):
    ...
    if qtu_dict['Detector'][2] == 'ETD':
        kw = '\nGrid='
        start = text.find(kw) + len(kw)
        end = text[start:].find('\n')
        Grid_Voltage = np.double(text[start : start + end])
        qtu_dict['Grid_Voltage'] = ['np.float64', 'V', Grid_Voltage]

    return qtu_dict


def readable_names(res, qtu):  # noqa: PLR0915
    nice_names = dict()
...
def create_data_json(  # noqa: PLR0912, PLR0915
    filepath,
    schemapath,
    data_name,
    nomad_sample_paths=None,
    ...
    data_json = dict()
    data_json['data'] = dict()
    print(qtu_dict['Detector'])
    if qtu_dict['Detector'][2] == 'ETD':
        data_json['data']['m_def'] = (
            '../upload/raw/Schemas/Data_Entries.archive.yaml#/definitions/section_definitions/1'
        )
        quantities += sections['SEM_Image_ETD']['quantities']
    elif qtu_dict['Detector'] == 'TLD':
        data_json['data']['m_def'] = (
            '../upload/raw/Schemas/Data_Entries.archive.yaml#/definitions/section_definitions/2'
        )
        quantities += sections['SEM_Image_TLD']['quantities']
    else:
        data_json['data']['m_def'] = (
            '../upload/raw/Schemas/Data_Entries.archive.yaml#/definitions/section_definitions/0'
        )
